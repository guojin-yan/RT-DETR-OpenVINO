...
namespace rt_detr_time_text
{
    public class RTDETRPredictor
    {
         ...
         string device_name = "CPU", bool postprcoess = true)
        {
            ...
            load_model += end.Subtract(start).TotalMilliseconds;
       ...
       /// </returns>
        public Mat predict(Mat image)
        {
            ...
            DateTime end = DateTime.Now;
            process_image += end.Subtract(start).TotalMilliseconds;
            start = DateTime.Now;
            if (post_flag)
            {
            ...
            load_data += end.Subtract(start).TotalMilliseconds;
            start = DateTime.Now;
            infer_request.infer();
            end = DateTime.Now;
            infer += end.Subtract(start).TotalMilliseconds;
            start = DateTime.Now;
            ResultData results;
            if (post_flag)
            {
                Tensor output_tensor = infer_request.get_tensor("reshape2_69.tmp_0");
            ...
            postprocess += end.Subtract(start).TotalMilliseconds;
       ...
       /// information about the model's inputs and outputs.</param>
        ...
        public static double load_model = 0;
        public static double process_image = 0;
        public static double load_data = 0;
        public static double infer = 0;
        public static double postprocess = 0;
